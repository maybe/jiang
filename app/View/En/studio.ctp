<div id="studio-content" class="centered" style="<?php 
    $size = getimagesize(WWW_ROOT."/img/studio/".$studio[$index]); echo "height:".($size[1]+100)."px;margin-left:-400px;padding-bottom:50px;margin-top:-".($size[1]/2+50)."px;position:absolute;top:50%;left:50%;"; ?>">
  <div id="img-context">
  <?php 
    ...
    <div id="left-arrow" next="<?php echo $index==0?"":"/en/studio?index=".($index-1) ?>"></div>
    <div id="right-arrow" next="<?php echo $index==count($studio)-1?"":"/en/studio?index=".($index+1) ?>"></div>
    </div>
  <?php } else {?>
